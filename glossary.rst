...
.. glossary::
   ...
   隐式转换
      在要求两个运算数类型相同的运算中，一种类的实例转化为另外一种类的实例。例如，``int(3.15)`` 把浮点数转化为整数 ``3``，但是在 ``3+4.5`` 中，运算数的类型各不相同(一个 int，一个 float)，所以在它们相加之前都要转化为相同的类型，否则就会抛出 ``TypeError``来异常。如果没有隐匿转换，即使是类型兼容的运算数也必须由程序员来转换。例如要写成 ``float(3)+4.5`` 而不是简单的 ``3+4.5``。
