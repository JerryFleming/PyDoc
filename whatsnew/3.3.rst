...
to properly delineate between :term:`meta path finders <元路径查找器>`
and :term:`path entry finders <路径条目查找器>` by introducing
...
new API, we are releasing them in Python 3.3 as a :term:`provisional policy <临时包>`.  Backwards incompatible changes (up to and including
...
The :mod:`sys` module has a new :data:`~sys.thread_info` :term:`结构体序列` holding informations about the thread implementation
(:issue:`11223`).
...
* :class:`importlib.abc.Finder` no longer specifies a `find_module()` abstract
  ...
  case of working with :term:`path entry finders <路径条目查找器>`.
