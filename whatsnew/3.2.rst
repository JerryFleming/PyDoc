...
resources.  The class is a :term:`上下文管理器` and can be used in a
...
* The :mod:`importlib.abc` module has been updated with new :term:`abstract base classes <虚基类>` for loading bytecode files.  The obsolete
...
* There is also a new :meth:`str.format_map` method that extends the
  ...
  :term:`映射` objects.  This new method makes it possible to use string
...
* The internal :c:type:`structsequence` tool now creates subclasses of tuple.
  ...
  :term:`命名元组` and now work with functions and methods that
...
* A new warning category, :exc:`ResourceWarning`, has been added.  It is
  ...
  A :exc:`ResourceWarning` is also issued when a :term:`文件对象` is destroyed
...
* :class:`range` objects now support *index* and *count* methods. This is part
  ...
  :class:`collections.Sequence` :term:`虚基类`.  As a result, the
...
* The :func:`callable` builtin function from Py2.x was resurrected.  It provides
  a concise, readable alternative to using an :term:`虚基类` in an
...
In addition, more classes now implement a :term:`上下文管理器` to support
...
These tools make it possible to define an :term:`虚基类` that
...
:term:`上下文管理器` that does double duty as a function decorator.
...
:term:`虚基类` (except for ``truncate()``).  It also has a
...
be a :term:`关键字参数 <实参>`.  The user-supplied filter function accepts a
...
allowed an optional *body* argument so that a :term:`文件对象` could be used
to supply the content of the request.  Conveniently, the *body* argument now
also accepts an :term:`可迭代对象` object so long as it includes an explicit
...
The :func:`~urllib.parse.urldefrag` function now returns a :term:`命名元组`::
...
* The mechanism for serializing execution of concurrently running Python threads
  (generally known as the :term:`GIL` or :term:`解释器全局锁`) has
...
* The `Timsort algorithm <http://en.wikipedia.org/wiki/Timsort>`_ used in
  ...
  when called with a :term:`键函数`.  Previously, every element of
